...
def step_environment(env, policy, ds, reset_after_n_steps, render=False, timing=False, **kwargs):
    """
    ...
    while True:
        ...
        global_step += 1
        epi_len = mean(info['epi_len'] + [0])
        epi_returns = mean(info['returns'] + [0.])
...
class VectorStateBufferDataset:
    def __init__(self, maxlen=None, statebuffer=None):
        self.buffer = deque(maxlen=maxlen)
        self.statebuffer = statebuffer if statebuffer is not None else VectorStateBuffer()
